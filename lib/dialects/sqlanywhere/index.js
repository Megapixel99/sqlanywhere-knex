...
class Client_Sqlanywhere extends Client {
  ...
  _query(connection, obj) {
    ...
    const ret = new Promise((resolver, rejecter) => {
      if (obj.bindings) {
        return connection.exec(obj.sql, obj.bindings, (execErr, result) => {
          if (execErr) return rejecter(execErr);
          resolver(result);
        });
      }
      return connection.exec(obj.sql, (execErr, result) => {
        if (execErr) return rejecter(execErr);
        resolver(result);
      });
    });
